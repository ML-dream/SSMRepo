...
<!-- 	</div> -->
    ...
    <div id="grid1" class="mini-datagrid" style="width:100%;height:85%;"
        ...
        <div property="columns">
            ...
            <div name="action" width="20" headerAlign="center" align="center" renderer="onOperatePower"
                 cellStyle="padding:0;">完工上报
            </div>
        <div field="orderId" width="50" allowSort="true" headerAlign="center" align="center">订单编号
            </div> 
         
            <div field="companyName" width="50" headerAlign="center" align="center">客户名称
            </div>
            ...
            <div field="completedAdvice" width="60" headerAlign="center"   align="center" >完工审核意见
            </div>
    ...
    <script type="text/javascript">
	    ...
	    grid.load({bookStatus: "13",sign:"uploadSelect"});
...
//          str += "</span>";
	        str += "<span>";
	        str += "<a style='margin-right:2px;' title='完工上报' href=javascript:ondMenu(\'"+e.row.orderId+"','"+e.row.companyName+"','"+e.row.connectorTel+"','"+e.row.deptUser+"','"+e.row.bookStatus+"','"+mini.formatDate(e.row.createTime,"yyyy-MM-dd-HH:mm:ss")+"','"+e.row.connector+"\') ><span class='icon-upload' style='width:30px;height:20px;display:inline-block'></span></a>";
	        str += "</span>";
	        return str;
	    }
		...
		function ondMenu(orderId,companyName,connectorTel,deptUser,bookStatus,createTime,connector){
	    	...
	    	window.location="ResearchTask/CompletedOrderSubmitDetail.jsp?orderId="+orderId+"&companyName="+companyName+"&connector="+connector+"&connectorTel="+connectorTel+"&deptUser="+deptUser+"&bookStatus="+bookStatus+"&createTime="+createTime;
		}
